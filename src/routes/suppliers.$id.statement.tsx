...
type Row = {
  date: string;
  type: "Opening" | "Purchase" | "Payment" | "Cash In" | "Purchase Return";
  ref: string;
  debit: number;
  credit: number;
  notes?: string;
...
function Statement() {
  const { id } = useParams({ from: "/suppliers/$id/statement" });
  const { db } = useStore();
  ...
  const { rows, openingBefore, totalDebit, totalCredit } = useMemo(() => {
    if (!s) return { rows: [] as Row[], openingBefore: 0, totalDebit: 0, totalCredit: 0 };
    const fromTs = new Date(from + "T00:00:00").getTime();
    const toTs = new Date(to + "T23:59:59.999").getTime();

    const purs = db.purchases.filter((p) => p.supplierId === id);
    const pays = db.supplierPayments.filter((p) => p.supplierId === id);
    const rets = db.purchaseReturns.filter((r) => r.supplierId === id);

    let opening = s.openingBalance;
    for (const p of purs) {
      const t = new Date(p.date).getTime();
      if (t < fromTs) opening += p.total - p.paid;
    }
    for (const p of pays) {
      const t = new Date(p.date).getTime();
    ...
    const totalDebit = inRange.reduce((sum, r) => sum + r.debit, 0);
    const totalCredit = inRange.reduce((sum, r) => sum + r.credit, 0);
    return { rows: inRange, openingBefore: opening, totalDebit, totalCredit };
  }, [s, db.purchases, db.supplierPayments, db.purchaseReturns, from, to, id]);
  ...
  function buildBodyHtml() {
    const rowsHtml = rows
      .map((r, idx) => {
        const run =
          openingBefore + rows.slice(0, idx + 1).reduce((sum, x) => sum + x.debit - x.credit, 0);
        return `<tr>
    ...
    return `
      <div class="head">
        ...
        <div style="text-align:right">
          <div class="label">Supplier Statement</div>
      ...
      <div class="grid2">
        <div>
          ...
          ${s!.company ? `<div>${escapeHtml(s!.company)}</div>` : ""}
          ${s!.address ? `<div class="muted">${escapeHtml(s!.address)}</div>` : ""}
          ${s!.phone ? `<div class="muted">${escapeHtml(s!.phone)}</div>` : ""}
        </div>
        <div style="text-align:right">
      ...
      <div class="grid3">
        <div class="box"><div class="label">Debit (Purchases)</div><div class="value">${fmtRound(totalDebit)}</div></div>
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      const safeName = s!.name.replace(/[^a-z0-9-_ ]/gi, "_");
      pdf.save(`Supplier_Statement_${safeName}_${from}_${to}.pdf`);
    } finally {
      container.remove();
    }
  }
  ...
  return (
    <>
      <PageHeader
        title={`Statement — ${s.name}`}
        subtitle="Supplier account statement"
        actions={
          <>
            <Link to="/suppliers">
              <Button variant="outline">
            ...
            <WhatsAppButton
              ...
              fileName={`Supplier_Statement_${s.name.replace(/[^a-z0-9-_ ]/gi, "_")}_${from}_${to}.pdf`}
              buildPdf={() =>
                buildStatementPdf({
                  company: db.company,
                  customerName: s.name,
        ...
        <div className="print-area mx-auto bg-card text-card-foreground border border-border rounded-md p-10 print:border-0 print:rounded-none print:p-0 max-w-[210mm] print:max-w-none print:w-full">
          ...
          <div className="grid grid-cols-2 gap-8 py-5">
            <div>
              ...
              {s.company && <div className="text-sm">{s.company}</div>}
          ...
          <div className="grid grid-cols-3 gap-3 pb-5">
            <SummaryBox
              label="Debit Balance"
              hint="Total purchases in period"
              value={fmtRound(totalDebit)}
            />
            <SummaryBox
              label="Credit Balance"
              hint="Total paid in period"
              value={fmtRound(totalCredit)}
              tone="text-success"
            />
            <SummaryBox
              label="Balance"
              hint="Closing balance (we owe)"
