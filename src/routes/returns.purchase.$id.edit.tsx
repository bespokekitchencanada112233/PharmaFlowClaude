...
export const Route = createFileRoute("/returns/purchase/$id/edit")({
  component: EditPurchaseReturn,
});

function EditPurchaseReturn() {
  const { id } = useParams({ from: "/returns/purchase/$id/edit" });
  const { db, updatePurchaseReturn } = useStore();
  const navigate = useNavigate();
  const ret = db.purchaseReturns.find((r) => r.id === id);

  const [supplierId, setSupplierId] = useState<string>(ret?.supplierId ?? "");
  ...
  function save() {
    ...
    updatePurchaseReturn(ret.id, { supplierId, items, notes: notes || undefined, date: iso });
    toast.success("Purchase return updated");
    navigate({ to: "/returns/purchase/$id", params: { id: ret.id } });
  }
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Update items, qty, cost, date or notes · Ctrl+S save"
        actions={
          <Link to="/returns/purchase/$id" params={{ id: ret.id }}>
