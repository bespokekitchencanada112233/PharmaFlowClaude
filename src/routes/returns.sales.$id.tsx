...
export const Route = createFileRoute("/returns/sales/$id")({
  component: SalesReturnView,
});

function SalesReturnView() {
  const { id } = useParams({ from: "/returns/sales/$id" });
  ...
  const ref = `SR-${ret.id.slice(0, 8).toUpperCase()}`;
  ...
  function downloadPdf() {
    ...
    a.href = url; a.download = `SalesReturn_${ref}.pdf`;
          ...
          <>
            ...
            <Button variant="outline" onClick={() => {
              if (confirm("Delete this return and revert stock?")) {
                deleteSalesReturn(ret.id); toast.success("Deleted");
                navigate({ to: "/returns" });
              }
        ...
        <div className="print-area mx-auto max-w-4xl bg-card text-card-foreground border border-border rounded-md p-10">
          <div className="flex justify-between items-start pb-6 border-b border-border">
            ...
            <div className="text-right">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">Sales Return</div>
          ...
          <div className="grid grid-cols-2 gap-8 py-6">
            <div>
              <div className="text-xs uppercase tracking-wide text-muted-foreground mb-1">From Customer</div>
